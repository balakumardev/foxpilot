...
describe("performInputAction", () => {
  ...
  describe("fill", () => {
    ...
    it("drives a checkbox via a real click (fires input + change once, ends checked)", () => {
      // A2: React binds a checkbox's onChange to the native click (its
      // ChangeEventPlugin uses shouldUseClickEvent), so filling a checkbox now
      // dispatches the covert click sequence — which toggles the box and fires
      // input + change exactly once each — instead of assigning `.checked`.
      document.body.innerHTML = `<input type="checkbox" />`;
      const cb = document.querySelector("input")!;
      stamp(cb, "e1");
      const onChange = jest.fn();
      ...
      // The state is reached by a real click (what React's ChangeEventPlugin
      // listens for), so the click handler runs exactly once.
      expect(onClick).toHaveBeenCalledTimes(1);
    });

    it("does NOT click a checkbox already in the requested state (idempotent fill)", () => {
      // cur === target → no click, no toggle, no change/input.
    ...
    it("selects a radio via a real click (fires change once, ends checked)", () => {
      ...
      expect(onChange).toHaveBeenCalledTimes(1);
      // Radio selection is reached by a real click (what React observes).
      expect(onClick).toHaveBeenCalledTimes(1);
    });
