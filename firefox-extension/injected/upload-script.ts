 ...
 * implementation built that DataTransfer inside a page-world <script> element it
 * injected — which a strict page Content-Security-Policy (e.g. the Chrome Web
 * Store dashboard) blocks, so the upload silently timed out.
 *
 ...
 *   1. Drop-zone resolution. Many upload widgets hide the real <input type=file>
 *      (display:none / 0×0) behind a styled "drop here" button or div. Hidden
 *      inputs are excluded from snapshots, so the only targetable element is the
 *      drop zone. We therefore resolve the input from the targeted element, a
 *      descendant, or a nearby ancestor.
 *   2. Firefox Xray. Firefox content scripts see the page through Xray wrappers;
 *      a FileList built with the isolated world's constructors is not always
 *      accepted by `input.files`. When `window.wrappedJSObject` + `cloneInto`
 *      are present (Firefox), we build the File/DataTransfer in the PAGE realm so
 *      the resulting FileList is one the input accepts. On Chrome the direct
 *      constructors work, so we use them.
 *
 * The MCP server has already read the file off disk and passed its bytes here as
 * base64 — the extension never sees a path.
 */
