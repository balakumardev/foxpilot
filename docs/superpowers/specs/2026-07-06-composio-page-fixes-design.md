...
## Open risks
- **Implementation refinement (shipped):** CSP detection uses a synchronous **"started" marker** written by the injected script rather than `securitypolicyviolation`. On Chrome the inline `<script>` runs synchronously on `appendChild`, so a missing marker is an instant, definitive CSP block; the 10s `EVAL_TIMEOUT_MS` backstop is **kept unchanged** (a legit async eval awaiting a fetch still needs it) — the marker fails fast *without* shortening it. On Firefox the marker is polled for a bounded 1s window. This is uniform across both extensions and avoids the cross-world/Xray uncertainty of `securitypolicyviolation`.
- **Nav-race coincidental-navigation edge (accepted, LOW):** `raceInputAgainstNavigation` keys on `tabs.onUpdated(status:"loading")`, which is not provably caused by the click. If a page is *already* mid-navigation (e.g. a multi-hop redirect page) and a top-frame load event fires within the ~ms window of a **failed** click (stale uid / off-point), the nav can win the race and report `{ok:true, navigated:true}`, masking the failure. Probability is low (millisecond window + requires a failed click during an active redirect) and it is design-inherent to `tabs.onUpdated`; a grace-period mitigation was considered and deliberately not added (complexity/risk outweighed the LOW severity). The common cases are correct: a non-navigating click's fast ack wins the race; a genuine navigating click's lost ack correctly becomes `navigated:true`.
- **CDP-eval never-resolving promise (accepted):** with `awaitPromise:true`, a page function that awaits a promise that never settles leaves the debugger attached until it resolves or the tab closes; the 30s broker timeout rejects the request server-side but cannot force the extension's `finally` detach. `tabs.onRemoved → forceDetachDebugger` reclaims it on tab close. A failed *attach* (e.g. DevTools already open) now returns a friendly `ok:false` and never detaches (nothing was attached).
